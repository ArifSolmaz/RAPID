"""
Phase 2 step 1: run RAPID-style inference on Pleiades A stars and
compare against the cluster's known age.
Run from repo root:
    python scripts\03_pleiades_validation.py
"""
import numpy as np
import pandas as pd
...
CLUSTER_AGE_MYR, CLUSTER_AGE_ERR = 125.0, 8.0   # Stauffer+98 LDB
...
bench = pd.read_csv(BENCH / "pleiades_seed.csv")
results = []
for _, row in bench.iterrows():
    w = infer(row.teff_K, row.teff_err_K, row.lum_Lsun, row.lum_err_Lsun)
    if w is None:
        print(f"  {row['name']:18s}  no posterior (off-grid)")
        continue
    ...
    print(f"  {row['name']:18s}  age = {a50:6.0f} (-{a50-a16:3.0f},+{a84-a50:3.0f}) Myr   "
...
res.to_csv(BENCH / "pleiades_results.csv", index=False)
...
ax.axhspan(CLUSTER_AGE_MYR - CLUSTER_AGE_ERR,
           CLUSTER_AGE_MYR + CLUSTER_AGE_ERR,
           color='green', alpha=0.18, label='Pleiades age (Stauffer+98)')
ax.axhline(CLUSTER_AGE_MYR, color='green', lw=1)
ax.set_xticks(x); ax.set_xticklabels(res.name, rotation=45, ha='right')
ax.set_ylabel('RAPID age (Myr)')
ax.set_title('Pleiades A-star ages — RAPID vs cluster (Stauffer+98)')
ax.legend()
fig.tight_layout()
fig.savefig(FIGS / '04_pleiades_validation.png', dpi=150)
print("wrote", FIGS / '04_pleiades_validation.png')